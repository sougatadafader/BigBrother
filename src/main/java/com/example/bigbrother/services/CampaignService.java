...
public class CampaignService {
	...
	public Campaign createCampaign(@RequestBody Campaign campaign, @PathVariable("dependentId") int id,
			HttpSession session) {
	...
	public boolean addCampaignToFavorite(@PathVariable("cId") int cId, @PathVariable("uId") int uId) {
	...
	public boolean isAddedToFavoriteByUser(@PathVariable("cId") int cId, @PathVariable("uId") int uId) {
		User user = userRepository.findById(uId).get();
		...
		Collections.sort(camps, new Comparator() {
			...
			public int compare(Object o1, Object o2) {
				return ((((Campaign) o1).getUsersWhoLiked().size()) < ((Campaign) o2).getUsersWhoLiked().size()) ? 1
						: ((((Campaign) o1).getUsersWhoLiked().size()) > ((Campaign) o2).getUsersWhoLiked().size()) ? -1
								: 0;
